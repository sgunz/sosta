...
> # TODO:  have to bandwith for all images the same
...
all_images <- colData(spe)$image_name |> unique()

bndws <- lapply(all_images, function(x){
  pp <- SPE2ppp(spe, marks = "cell_category", image_col = "image_name", image_id = x)
  bndw <- bw.diggle(pp)
  return(bndw)
})
